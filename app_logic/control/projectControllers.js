...
module.exports = {
  create: async (req, res) => {
    try {
      ...
      let results = await db.exec("createProject", {
        projectname,
        username,
        project_description,
      });
  ...
  getOneProject: async (req, res) => {
    try {
      ...
      let results = await db.exec("getProject", { projectname });
  ...
  getAllProjects: async (req, res) => {
    try {
      let { recordset } = await db.query("select * from project.project_list");
  ...
  deleteProject: async (projectname, done) => {
    try {
      let results = await db.exec("deleteProject", { projectname });
      console.log(results.recordset);
  ...
  updateProject: async (projectname, projectid, project_description, done) => {
    try {
      let results = await db.exec("updateProject", {
        projectname,
        projectid,
        project_description,
      });
